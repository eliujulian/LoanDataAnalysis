...
cat_names <- c("Not Available", "Debt Consolidation", "Home Improvement",  
               ...
               "Wedding Loans")
ld$ListingCategory..numeric.rev <- 
  with(ld, factor(ListingCategory..numeric., 
                  levels = rev(levels(ListingCategory..numeric.))))
ggplot(ld, aes(x = ListingCategory..numeric.rev)) + 
  geom_bar(fill = myColor, alpha = 0.8) + 
  scale_x_discrete(labels = rev(cat_names)) +
  coord_flip()
  # theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
grid.arrange(
  ggplot(ld, aes(x = StatedMonthlyIncome)) + 
    ...
    xlim(0, quantile(ld$StatedMonthlyIncome, 0.99)) + 
    ggtitle("Histogram of Stated Incomes (Binwidth = 10)"),
  ggplot(ld, aes(x = StatedMonthlyIncome)) + 
    geom_histogram(binwidth = 500, fill = myColor, alpha = 0.9) + 
    xlim(0, quantile(ld$StatedMonthlyIncome, 0.99)) + 
    ggtitle("Histogram of Stated Incomes (Binwidth = 500)")
)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=10}
ld.without_current$ListingCategory..numeric.rev <- 
  with(ld.without_current, 
       factor(ListingCategory..numeric., 
              levels = rev(levels(ListingCategory..numeric.))))
ggplot(ld.without_current, 
       aes(x = ListingCategory..numeric.rev, fill = LoanStatus.Bucket)) + 
  geom_histogram(stat = "count") + 
  scale_x_discrete(labels = rev(cat_names)) +
  coord_flip()
```
...
ggplot(subset(ld.without_current, Investors > 1), 
       aes(x = Investors, color = LoanStatus.Bucket)) + 
...
ggplot(ld.without_current, aes(x = LoanOriginalAmount, 
  ...
  ylim(0, quantile(ld$StatedMonthlyIncome, 0.95)) +
  scale_color_gradient(low = "plum", high = "black") + 
  facet_wrap(~LoanStatus.Bucket)
```
