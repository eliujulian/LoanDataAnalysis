...
ld$ProsperScore <- as.factor(ld$ProsperScore) # Changing ProsperScore to a factor since the numbers represent groups.
ld$ListingCategory..numeric. <- as.factor(ld$ListingCategory..numeric.) # Changing to factor since number represent categories
ld$DateCreditPulled <- as.Date(ld$DateCreditPulled) # Converting strings to Dates
ld$ListingCreationDate <- as.Date(ld$ListingCreationDate) # Converting strings to Dates
ld$LoanOriginationDate <- as.Date(ld$LoanOriginationDate)  # Convert strings to Dates
ld$IsUnique <- !duplicated(ld$ListingKey) #Remove Double entries
...
```{r echo=FALSE}
ggplot(data = as.data.frame(table(ld$LoanStatus)), aes(x = reorder(Var1, -Freq), y = Freq, fill = Freq)) + 
...
ld$LoanStatus.Bucket[ld$LoanStatus.Bucket == "Chargedoff" | ld$LoanStatus.Bucket == "Defaulted" ] <- "Not successful"
ld$LoanStatus.Bucket[ld$LoanStatus.Bucket != "successful" & ld$LoanStatus.Bucket != "Not successful" ] <- "Open"
...
ld$IsEmployed <- ifelse(ld$EmploymentStatus %in% employment_stat, "Employed", "Not employed")
ld$IsEmployed <- as.factor(ld$IsEmployed)

ld.without_current <- subset(ld, ld$LoanStatus.Bucket != "Open")
ld.without_current$LoanStatus.Bucket <- as.character(ld.without_current$LoanStatus.Bucket)
ld.without_current$LoanStatus.Bucket <- as.factor(ld.without_current$LoanStatus.Bucket)
```
...
```{r echo=FALSE}
ggplot(data = as.data.frame(table(ld$LoanStatus.Bucket)), aes(x = reorder(Var1, -Freq), y = Freq, fill = Freq)) + 
...
```{r echo=FALSE}
grid.arrange(
  ggplot(ld, aes(x = BorrowerRate)) + geom_histogram(binwidth = 0.007, fill = 'blue', alpha = 0.5),
  ggplot(ld, aes(x = LenderYield)) + geom_histogram(binwidth = 0.007, fill = 'red', alpha = 0.5)
  )
```
...
cat_names <- c("Not Available", "Debt Consolidation", "Home Improvement",  "Business","Personal Loan", "Student Use", "Auto", "Other", "Baby&Adoption", "Boat", "Cosmetic Procedure", "Engagement Ring", "Green Loans", "Household Expenses", "Large Purchases", "Medical/Dental", "Motorcycle", "RV", "Taxes", "Vacation", "Wedding Loans")
...
```{r echo=FALSE}
ggplot(as.data.frame(sort(table(ld$Occupation), decreasing = TRUE)[0:10]), aes(x = Var1, y = Freq)) + 
...
```{r echo=FALSE}
ggplot(as.data.frame(table(subset(ld, !is.na(EmploymentStatus))$EmploymentStatus)), aes(x = reorder(Var1, -Freq), y = Freq)) +
  geom_bar(stat = "identity", fill = myColor, alpha = 0.8) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
...
```{r echo=FALSE}
ggplot(as.data.frame(table(ld$IsEmployed)), aes(x = "", y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity")+
  coord_polar("y", start = 0) + 
  scale_fill_brewer(palette="Blues", guide = guide_legend(title = "Status")) +
  theme_minimal() +
  xlab("") + 
  ylab("")
```
...
* LoanOriginalAmount
Most loans were in a lower range. The median is 6,300 USD and the mean 8,135 USD.

* MonthlyLoanPayment
Most loan payments are below 370 USD per month (3rd quartile). There is an unexpected spike at 170 USD per month.

* Loan Status
Factor that shows if the loan is current, if it was payed back or if the borrower did not pay it pack.
...
ld.without_current %>%
  ...
  summarise(mean_stated_income = mean(StatedMonthlyIncome),
            ...
            mean_debt_to_inc_ratio = mean(DebtToIncomeRatio, na.rm = TRUE),
            mean_duration_employment = mean(EmploymentStatusDuration, na.rm = TRUE),
            n = n())
```
...
```{r echo=FALSE, warning=FALSE,message=FALSE}
ggplot(ld.without_current, aes(x = LoanStatus.Bucket, y = StatedMonthlyIncome)) + 
  geom_boxplot(fill = myColor, alpha = 0.5) +
  ylim(-1, 25000)
```

```{r echo=FALSE}
wilcox.test(StatedMonthlyIncome ~ LoanStatus.Bucket, data = ld.without_current, conf.int=TRUE)
```
...
```{r echo=FALSE}
wilcox.test(LoanOriginalAmount ~ LoanStatus.Bucket, data = ld.without_current, conf.int = TRUE)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(ld.without_current, aes(x = Term)) + 
  geom_histogram(fill = myColor) + 
  facet_wrap(~LoanStatus.Bucket)
```
...
ld.without_current %>%
  ...
  summarise(
    success_ratio = sum(LoanStatus.Bucket == "successful") / 
      (sum(LoanStatus.Bucket == "successful") + sum(LoanStatus.Bucket != "successful")),
    n = n()
  )
```
...
```{r echo=FALSE}
wilcox.test(BorrowerRate ~ LoanStatus.Bucket, data = ld.without_current, conf.int = TRUE)
```
...
```{r echo=FALSE}
chisq.test(table(ld.without_current$LoanStatus.Bucket, ld.without_current$IsEmployed))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(ld.without_current, aes(x = LoanStatus.Bucket, y = Investors)) + 
  geom_boxplot(fill = myColor, alpha = 0.5)
```
...
```{r echo=FALSE}
wilcox.test(Investors ~LoanStatus.Bucket, data = ld.without_current, conf.int=TRUE)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ld.without_current$HasInvestmentFromFriends <- ld.without_current$InvestmentFromFriendsCount >= 1
ld.without_current %>%
...
ggplot(ld.without_current, aes(x = LoanStatus.Bucket, fill = LoanStatus.Bucket)) + 
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=10}
ggplot(ld.without_current, aes(x = ListingCategory..numeric., fill = LoanStatus.Bucket)) + 
  geom_histogram(stat = "count") + 
  scale_x_discrete(labels = cat_names) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
ld.without_current$ListingCategory..names. <- ld.without_current$ListingCategory..numeric.
ld.without_current$ListingCategory..names. <- names(cat_names.code)[match(ld.without_current$ListingCategory..names., cat_names.code)]
ld.without_current %>%
...
ggplot(tmp, aes(x = reorder(Occupation, -success_ratio), y = success_ratio, fill = success_ratio)) + 
  geom_bar(stat= "identity") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
summary(subset(ld.without_current, Occupation == "Military Officer")$BorrowerRate)
```
...
```{r echo=FALSE}
chisq.test(table(ld.without_current$IsBorrowerHomeowner, ld.without_current$LoanStatus.Bucket))
```
...
ggplot(ld, aes(y = BorrowerRate, x = LoanOriginalAmount)) + 
  ...
  geom_line(aes(y = BorrowerRate, x = round(LoanOriginalAmount / 500) * 500), stat = 'summary', fun.y = mean , color = "red")
```
...
ggplot(ld, aes(x = StatedMonthlyIncome, y = BorrowerRate)) +
  ...
  geom_line(aes(y = BorrowerRate, x = round(StatedMonthlyIncome / 100) * 100), stat = 'summary', fun.y = mean , color = "red")
```
...
ggplot(ld, aes(x = MonthlyLoanPayment, y = BorrowerRate)) +
  ...
  geom_line(aes(y = BorrowerRate, x = round(MonthlyLoanPayment / 100) * 100), stat = 'summary', fun.y = mean , color = "red")
```
...
ggplot(ld, aes(y = LoanOriginalAmount, x = MonthlyLoanPayment, color = as.factor(Term))) + 
...
ggplot(ld, aes(x = LoanOriginalAmount, y = StatedMonthlyIncome, color = IsDeptRestructuring)) + 
  geom_point(alpha = 1 / 25) + 
  ylim(0, quantile(ld$StatedMonthlyIncome, 0.95)) + 
  geom_line(aes(y = StatedMonthlyIncome, x = round(LoanOriginalAmount / 500) * 500), stat = 'summary', fun.y = mean , color = "red")
```
...
ggplot(ld.without_current, aes(x = LoanOriginalAmount, y = StatedMonthlyIncome, color = BorrowerRate)) + 
...
grid.arrange(
  ggplot(subset(ld.without_current, LoanStatus.Bucket == "successful"), aes(x = BorrowerRate, y = StatedMonthlyIncome)) + 
    geom_point(alpha = 1 / 70) + 
    ylim(0, quantile(ld$StatedMonthlyIncome, 0.95)) + 
    ggtitle("successful Loans"),
  ggplot(subset(ld.without_current, LoanStatus.Bucket == "Not successful"), aes(x = BorrowerRate, y = StatedMonthlyIncome)) + 
...
ggplot(ld.without_current, aes(x = ProsperScore, y = StatedMonthlyIncome, color = LoanStatus.Bucket)) + 
  geom_boxplot() +
...
ggplot(subset(ld.without_current, !is.na(LoanStatus.Bucket)), aes(x = ProsperScore, y = BorrowerRate, color = LoanStatus.Bucket)) +
  geom_boxplot(alpha = 1 / 5)
  # ylim(0, quantile(ld.without_current$StatedMonthlyIncome, 0.99))
```
...
grid.arrange(
  ...
  ggplot(subset(ld.without_current, ld.without_current$ListingCategory..numeric. == 16), aes(x = IsEmployed, fill = LoanStatus.Bucket)) + 
    geom_bar() + 
    ggtitle("Listing Category Motor Cycle"),
  ggplot(subset(ld.without_current, ListingCategory..numeric. == 1), aes(x = IsEmployed, fill = LoanStatus.Bucket)) + 
    geom_bar() + 
    ggtitle("Listing Category Debt Restructuring"),
  ggplot(subset(ld.without_current, ListingCategory..numeric. == 0), aes(x = IsEmployed, fill = LoanStatus.Bucket)) + 
    geom_bar() + 
...
ggplot(subset(ld.without_current, Investors > 1), aes(x = Investors, fill = LoanStatus.Bucket)) + 
